...
pub enum Op {
    ...
    // Jumps
    Fuse(usize), // Fuse next operation with a relative forward jump
    Forward(usize),  // Relative jump Phorward ;)
    Backward(usize), // Relative jump backward
...
impl Op {
    ...
    pub fn execute(ops: &[Op], context: &mut Context) -> Result<Accept, Reject> {
        ...
        impl Fuse {
            fn new(context: &Context, origin: usize, goto: usize) -> Fuse {
                assert!(origin < goto);
        ...
        let mut fuses: Vec<Fuse> = vec![
            Fuse::new(context, 0, ops.len())  // Main fuse for this block
        ];
        let mut ip = 0;
        ...
        while ip < ops.len() {
            ...
            let result = match op {
                ...
                // Jumps
                Op::Fuse(goto) => {
                    fuses.push(Fuse::new(context, ip, ip + goto + 1));
                    Ok(Accept::Skip)
                }
                ...
                Op::Forward(goto) => {
                    ...
                    // Remove any fuses with goto in front of new ip
                    fuses = fuses.into_iter().filter(|fuse| fuse.goto >= ip).collect();
                    Ok(Accept::Skip)
                }
                ...
                Op::Backward(goto) => {
                    ...
                    // Remove any fuses with origin in front of new ip.
                    fuses = fuses.into_iter().filter(|fuse| fuse.origin >= ip).collect();
                    Ok(Accept::Skip)
                }
        ...
        // At this point, the main fuse must still be alive.
        assert!(fuses.len() == 1);
        let fuse = fuses.pop().unwrap();

        match context.collect(fuse.capture_start, false, true, true, 0) {
