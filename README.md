...
Therefore, Tokay provides both a language for simple one-liners, but also features to create tools for code-analysis, refactoring, interpreters, compilers, transpilers or even entire domain specific languages. Even [Tokay's own language parser](examples/tokay.tok) is implemented with Tokay itself.
