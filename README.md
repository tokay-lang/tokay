...
There are currently two parser implementation approaches, the CBOR-based pre-compiled Tokay parser is used as the default.

- `make parser-cbor` updates the content of `src/compiler/_tokay.cbor` from `src/compiler/tokay.tok`.
- `make reset-parser-cbor` resets `src/compiler/_tokay.rs` from git, if something went wrong.

The old AST-based parser that is compiled internally into a Tokay VM program is still be supported. In the end, both parsers implement exactly the same program.
